...
In this project, we will perform data analysis to search for the most optimized model to predict the area of burned forest given a set of meteorological data. We will split the data into training and testing sets, construct a KNN model on the training set and evaluate its performance on the testing set. Additionally, we will discuss the implications of our results, things that we can improve on, and what future research questions our project can lead to. 
...
Given a set of meteorological and other data, the question that this project aims to answer is: **How large the burned area will be in the next year?**
...
We see that the final model's testing RMSE is roughly `r rmse_val` ha.
